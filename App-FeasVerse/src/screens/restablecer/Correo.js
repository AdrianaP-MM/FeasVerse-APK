...
const Correo = ({ navigation }) => {
    ...
    const handleSendCode = async () => {
        ...
        try {
            const response = await fetch(`${Config.IP}/FeasVerse-Api/api/services/publica/cliente.php?action=searchMail`, {
                method: 'POST',
                body: formData,
            });
            ...
            if (data.status) {
                ...
                const response2 = await fetch(`${Config.IP}/FeasVerse-Api/api/services/publica/cliente.php?action=enviarCodigoRecuperacion`, {
                    method: 'POST',
                    body: form2,
                });
